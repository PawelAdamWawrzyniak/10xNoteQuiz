...
8.  **Dedykowany Endpoint dla Powtórek**: Wprowadzono do planu API nowy endpoint `GET /notes/due-for-review`, aby efektywnie pobierać notatki gotowe do sesji powtórkowej.
9.  **Dynamiczne Tworzenie Tagów/Kategorii**: W formularzu edycji notatki zostanie zaimplementowany komponent typu combobox, który pozwoli na wybór istniejących tagów/kategorii oraz na ich dynamiczne tworzenie bez opuszczania formularza.
10. **Zarządzanie Stanem Serwera**: Do zarządzania stanem, buforowania danych (caching) i synchronizacji z API zostanie wykorzystana dedykowana biblioteka, taka jak React Query (TanStack Query).
11. **Scentralizowana Obsługa Błędów**: Zostanie stworzony mechanizm mapujący kody błędów API (np. 402, 409, 503) na zrozumiałe dla użytkownika komunikaty, wyświetlane w formie powiadomień (toastów) lub okien modalnych dla krytycznych błędów.
12. **Komunikacja nt. Klucza API**: Użytkownik będzie informowany o liczbie pozostałych darmowych quizów za pomocą stałego, ale możliwego do zamknięcia banera, który będzie zawierał również CTA kierujące do ustawień profilu w celu dodania własnego klucza API.
13. **Responsywność (Mobile-First)**: Architektura UI będzie tworzona zgodnie z podejściem "mobile-first", zapewniając czytelność i użyteczność kluczowych widoków, takich jak lista notatek (jedna kolumna) i edytor Markdown (uproszczony interfejs), na mniejszych ekranach.
14. **Dostępność (a11y)**: Niestandardowe, interaktywne komponenty (interfejs quizu, menedżer tagów) będą w pełni dostępne z poziomu klawiatury i zgodne z wytycznymi WAI-ARIA, aby zapewnić obsługę przez czytniki ekranu.
    </matched_recommendations>
